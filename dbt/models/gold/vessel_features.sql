...
-- MMSIs get reassigned/reused when a vessel changes flag or transponder (the IMO number is
-- the permanent hull identity; the MMSI is not, and AIS transmits the MMSI). In this dataset
-- all 5 MMSI hits were collisions (e.g. MMSI 249256000 is OFAC-listed 'SINA', flag Panama,
-- but actually transmits as 'LUIGI GALVANI', a legitimate Malta-flagged LPG tanker,
-- IMO 9738246 — verified against vessel registries). A real hit needs a second key to agree.
-- IMO would be strongest but the OpenSanctions vessel pull carries no IMO here, so corroborate
-- on the vessel NAME (normalised: upper-case, strip non-alphanumerics).

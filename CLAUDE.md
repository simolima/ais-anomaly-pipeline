...
- **Sanctions screening is MMSI-based (POC limitation)**: AIS↔OpenSanctions matching uses MMSI, which is reassignable/reused, so MMSI-only matches are false positives (in the 2024 data all 5 hits were collisions). `vessel_features.sql` corroborates on MMSI **+ normalised name** (`is_sanctioned`) and keeps `sanctions_mmsi_only_hit` for collisions to investigate. The robust fix — match on the permanent **IMO** number — is blocked because the OpenSanctions pull leaves `imo_number` null; populating it in `02_ingest_sanctions_bronze.py` is the real upstream fix. See `lessons-learned/2026-06-17_ml-feature-quality_sanctions-screening.md`.
- **ML feature quality depends on `vessel_features` filters**: `ais_impossible_speed` flags everything > 30 kn, which is dominated by GPS jitter, bad positions and fast-ferry cruise. `vessel_features.sql` isolates genuine position jumps via a **distance** filter (`distance_nm between 2 and 500 AND implied_speed_knots > 50`), not an implied-speed cap (which also drops real teleports). `vessel_features` is a full `table`, so it rebuilds in one `dbt run --select vessel_features --target prod` without re-running the windowed backfill.
